...
Create any missing directories silently. If `wiki/index.md` does not exist, create it before processing. See [vault-conventions.md](references/vault-conventions.md) for full vault structure, naming rules, and article format.
...
3. **Determine topic folder(s)** — map to existing topics in `wiki/` or create new ones. For cross-cutting insights spanning multiple topics, use `wiki/connections/`. For each new topic:
   - Create the folder `wiki/<topic-name>/` (lowercase kebab-case)
   - Create `wiki/<topic-name>/index.md` using the per-topic index format from [vault-conventions.md](references/vault-conventions.md)
...
5. **Update `wiki/<topic>/index.md`** — add the new article entry alphabetically: `- [[topic-name/article-name]] — one-line description`
6. **Update `wiki/index.md`** — if the topic is new, add it to the Topics section alphabetically; append a new row to the Articles table: `| [[path/article]] | one-line summary | source/file.md | YYYY-MM-DD |`
...
- Update all affected per-topic `index.md` files
