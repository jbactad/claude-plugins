...
Target under 200 lines per CLAUDE.md file. Bloated files cause Claude to ignore instructions. If a file exceeds this threshold, recommend:
- Splitting detailed content into `.claude/rules/*.md` files
- Using `@path/to/file` imports to reference external docs
- Removing generic advice Claude already knows

## Import Validation

CLAUDE.md supports `@path/to/file` import syntax. When found:
- Verify the imported file exists
- Check for circular imports (max 5 hops)
- Ensure imported content is still relevant
...
- Files exceeding 200 lines without using rules or imports
- Broken `@path` imports pointing to missing files
